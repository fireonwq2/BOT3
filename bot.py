...
async def server_start_task(interaction: discord.Interaction, container_name: str):
    userid = str(interaction.user.id)
    container_id = get_container_id_from_database(userid, container_name)

    if not container_id:
        await interaction.response.send_message(embed=discord.Embed(description="⚠️ No instance found for your user.", color=0xffcc00))
        return
    ...
    try:
        ...
        ssh_session_line = await capture_ssh_session_line(exec_cmd)
        if ssh_session_line:
            embed = discord.Embed(title="🚀 Instance Started!", color=0x00ff00)
            embed.add_field(name="🔑 SSH Command", value=f"```{ssh_session_line}```", inline=False)
            await interaction.user.send(embed=embed)
            await interaction.response.send_message(embed=discord.Embed(description="✅ Instance started successfully. Check your DMs for details.", color=0x00ff00))
        else:
            await interaction.response.send_message(embed=discord.Embed(description="⚠️ Instance started, but failed to get SSH session.", color=0xffcc00))
    except subprocess.CalledProcessError as e:
        await interaction.response.send_message(embed=discord.Embed(description=f"❌ Error starting instance: {e}", color=0xff0000))

@bot.tree.command(name="deploy-custom", description="🚀 Deploy a VPS with specific RAM, cores, and OS.")
@app_commands.describe(preset="🔧 Select a preset VPS or enter custom values", ram="💾 Custom RAM in GB", cores="💻 Custom CPU cores", os="📀 Operating System")
@app_commands.choices(
    preset=[
        app_commands.Choice(name="🟢 4GB RAM, 2 Core", value="4_2"),
        app_commands.Choice(name="🟢 6GB RAM, 2 Core", value="6_2"),
        app_commands.Choice(name="🟢 8GB RAM, 2 Core", value="8_2"),
        app_commands.Choice(name="🟢 10GB RAM, 2 Core", value="10_2"),
        app_commands.Choice(name="🟡 12GB RAM, 3 Core", value="12_3"),
        app_commands.Choice(name="🟡 16GB RAM, 4 Core", value="16_4"),
        app_commands.Choice(name="🔴 64GB RAM, 5 Core", value="64_5"),
        app_commands.Choice(name="⚙️ Custom", value="custom")
    ]
)
async def deploy_custom(interaction: discord.Interaction, preset: str, ram: int = None, cores: int = None, os: str = "ubuntu"):
    userid = str(interaction.user.id)

    if preset != "custom":
        ram, cores = map(int, preset.split("_"))

    if not ram or not cores:
        await interaction.response.send_message(embed=discord.Embed(description="⚠️ Invalid RAM or Core count. Please choose a valid configuration.", color=0xffcc00))
        return

    if userid not in whitelist_ids and count_user_servers(userid) >= SERVER_LIMIT:
        await interaction.response.send_message(embed=discord.Embed(description="❌ Instance limit reached. Upgrade to deploy more servers.", color=0xff0000))
        return

    try:
    container_id = subprocess.check_output([
        "docker", "run", "-itd", "--privileged", "--hostname", "testing",
        "--memory", f"{ram}g", "--cpus", str(cores), "--cap-add=ALL", f"{os}:latest"
    ]).strip().decode('utf-8')
        ...
        if ssh_session_line:
            embed = discord.Embed(title="🚀 VPS Deployed Successfully!", color=0x00ff00)
            embed.add_field(name="🖥️ OS", value=f"`{os}`", inline=True)
            embed.add_field(name="💾 RAM", value=f"`{ram}GB`", inline=True)
            embed.add_field(name="💻 CPU Cores", value=f"`{cores}`", inline=True)
            embed.add_field(name="🔑 SSH Command", value=f"```{ssh_session_line}```", inline=False)
            await interaction.user.send(embed=embed)

            add_to_database(userid, container_id, ssh_session_line)
            await interaction.response.send_message(embed=discord.Embed(description="✅ Instance created successfully. Check your DMs for details.", color=0x00ff00))
        else:
            await interaction.response.send_message(embed=discord.Embed(description="⚠️ Failed to retrieve SSH session.", color=0xffcc00))

    except subprocess.CalledProcessError as e:
        await interaction.response.send_message(embed=discord.Embed(description=f"❌ Error creating container: {e}", color=0xff0000))
