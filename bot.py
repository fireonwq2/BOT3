...
@bot.tree.command(name="deploy", description="Creates a new Instance with custom RAM and CPU settings")
@app_commands.describe(ram="Amount of RAM in MB (max: 16384)", cores="Number of CPU cores (max: 8)")
async def deploy(interaction: discord.Interaction, ram: int, cores: int):
    userid = str(interaction.user.id)

    # Define allowed RAM values
    allowed_ram_values = [1024, 2048, 3072, 4096, 5120, 8192, 12288, 16384]

    if ram not in allowed_ram_values:
        await interaction.response.send_message(embed=discord.Embed(
            description=f"Invalid RAM value. Allowed values: {', '.join([f'{r}MB' for r in allowed_ram_values])}",
            color=0xff0000
        ))
        return

    if cores > 8 or cores < 1:
        await interaction.response.send_message(embed=discord.Embed(
            description="Max allowed cores are 8 and minimum is 1.",
            color=0xff0000
        ))
        return
    ...
    if count_user_servers(userid) >= SERVER_LIMIT:
        await interaction.response.send_message(embed=discord.Embed(
            description="You have reached the server limit.",
            color=0xff0000
        ))
        return
    ...
    try:
        container_id = subprocess.check_output([
            ...
            f"--memory={ram}m", f"--cpus={cores}",
            "--hostname", "crashcloud", "--cap-add=ALL",
            "ubuntu-22.04-with-tmate"
        ]).strip().decode('utf-8')
        ...
        await interaction.response.send_message(embed=discord.Embed(
            description=f"### Error during deployment: {e}",
            color=0xff0000
        ))
