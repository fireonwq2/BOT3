...
async def create_server_task(interaction, ram="2g", cpu=2, ubuntu_version="22.04", password="root"):
    await interaction.followup.send(embed=discord.Embed(
    ...
    try:
        # 🛠️ Create Docker container with SSH support
        container_id = subprocess.check_output([
            "docker", "run", "-itd", "--privileged",
            "--hostname", "crashcloud", "--cap-add=ALL",
            "--memory", ram, "--cpus", str(cpu),
            "-p", "0.0.0.0:2222:22",  # Expose SSH port
            image
        ]).strip().decode('utf-8')

        # 🛠️ Install SSH server inside the container
        subprocess.run(["docker", "exec", container_id, "bash", "-c", "apt update && apt install -y openssh-server && service ssh start"], check=True)
    ...
    try:
        # Get SSH session command
        ssh_command = f"ssh root@your-server-ip -p 2222"
    except Exception as e:
        await interaction.followup.send(embed=discord.Embed(
            description=f"❌ **Error generating SSH command:** {e}", 
            color=0xff0000))
        subprocess.run(["docker", "kill", container_id])
        subprocess.run(["docker", "rm", container_id])
        return

    # ✅ Send VPS details to the user
